...
from github import auto_git_push
...
async def _resume_workflow(thread_id: str, db: AsyncSession) -> None:
    ...
    Also triggers GitHub push after successful approval.
    """
    config = {"configurable": {"thread_id": thread_id}}
    ...
    try:
        ...
        if approval_status == "approved":
            ...
            # ✅ GitHub Push AFTER blog is generated
            try:
                auto_git_push(GITHUB_REPO_DIR)
                logger.info(f"[BG] GitHub push completed thread='{thread_id}'")
            except Exception as git_err:
                logger.error(
                    f"[BG] GitHub push failed thread='{thread_id}': {git_err}",
                    exc_info=True
                )
        ...
        logger.error(
            f"[BG] Resume FAILED thread='{thread_id}': {e}",
            exc_info=True
        )
