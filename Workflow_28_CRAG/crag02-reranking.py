...
retriever    = vector_store.as_retriever(
    ...
    search_kwargs={"k": 10}   # Increased K so re-ranking can dynamically filter
)
...
# Dynamic Re-ranking: LLM judge (keep / drop per chunk) — fully dynamic, no static top-K
...
rerank_prompt = ChatPromptTemplate.from_messages(
    [
        (
            "system",
            "You are a strict relevance re-ranker.\n"
            "Return keep=true ONLY if the chunk directly helps answer the question.\n"
            "Use ONLY the chunk content. Output JSON only."
        ),
        ("human", "Question: {question}\n\nChunk:\n{chunk}")
    ]
)

rerank_chain = rerank_prompt | llm.with_structured_output(KeepOrDrop)


# State
class State(TypedDict):
    question: str
    docs: List[Document]       # Raw chunks from retriever
    kept_docs: List[Document]  # Dynamically kept chunks after re-ranking
    answer: str
...
# Re-rank Node (dynamic keep/drop logic)
def rerank(state: State) -> State:
    question = state["question"]
    kept: List[Document] = []

    for i, doc in enumerate(state["docs"]):
        decision = rerank_chain.invoke({
            "question": question,
            "chunk": doc.page_content
        }).keep

        log.debug(f"  Chunk {i+1}/{len(state['docs'])} {'KEPT' if decision else 'DROPPED'}: {doc.page_content[:80]}…")
        if decision:
            kept.append(doc)

    log.info(f"Re-rank — chunks kept: {len(kept)} / {len(state['docs'])}")
    return {"kept_docs": kept}


# Prompt for final generation (uses only dynamically kept chunks)
...
def generate(state: State) -> dict:
    context = "\n\n".join(d.page_content for d in state["kept_docs"]).strip()
...
# Build graph: start -> retrieve -> rerank -> generate -> end
...
app = graph.compile()
log.info("RAG with dynamic re-ranking graph compiled and ready.")
...
if __name__ == "__main__":
    ...
    Path("rag-with-rerank.png").write_bytes(png_bytes)
    log.info("Graph flow saved successfully as 'rag-with-rerank.png'")
    ...
    # ── Retrieved raw chunks (before re-ranking) ──────────────────
    ...
    # ── Kept chunks after dynamic re-ranking ─────────────────────
    kept = res["kept_docs"]
    log.info(f"Total chunks kept after re-ranking: {len(kept)} / {len(res['docs'])}")
    print("\n" + "=" * 60)
    print(f"Kept Chunks After Re-ranking ({len(kept)} of {len(res['docs'])}):")
    print("=" * 60)
