...
def _build_vector_store() -> FAISS:
    ...
    for pdf_path in BOOK_FILES:
        ...
        # ← PDFPlumberLoader replacement (drop-in replacement that handles tables/context much better)
...
class State(TypedDict):
    ...
    strips: List[str]         # Output of decompose (sentences strips)
    kept_strips: List[str]    # Selected Strips after filtering
...
# Sentence-Level Decomposition Node
...
# Refining LLM Node: Decompose -> Filter -> Recompose
def refine(state: State) -> State:
    question = state["question"]
    context = "\n\n".join(d.page_content for d in state["docs"]).strip()

    # 1. Decompose context into sentences
    strips = decompose_to_sentences(context)
    log.info(f"Refine — total sentences decomposed: {len(strips)}")

    # 2. Filter sentences with LLM judge
    kept: List[str] = []
    ...
    log.info(f"Refine — sentences kept: {len(kept)} / {len(strips)}")

    # 3. Recompose kept sentences into refined context
    refined_context = "\n".join(kept).strip()
    log.debug(f"Refined context length: {len(refined_context)} chars")
...
if __name__ == "__main__":
    ...
    # ── Retrieved raw chunks (before refine) ──────────────────
    log.info(f"Total raw chunks retrieved: {len(res['docs'])}")
    print("\n" + "=" * 60)
    print("Retrieved Chunks (before refine):")
    print("=" * 60)
    for i, doc in enumerate(res["docs"]):
        log.debug(f"Printing raw chunk {i+1}")
        print(f"\n--- Chunk {i+1} ---")
        print(doc.page_content)

    # ── Kept sentences (after refine) ─────────────────────────
    kept = res["kept_strips"]
