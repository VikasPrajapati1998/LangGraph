...
def research_node(state: State) -> dict:
    ...
    # ── Point 1: run all Tavily searches concurrently (I/O-bound → threads) ──
    raw_results: List[dict] = []
    search_start = datetime.now()
    with ThreadPoolExecutor(max_workers=min(len(queries), 10)) as pool:
        future_to_query = {
            pool.submit(_tavily_search, q, max_results): q
            for q in queries
        }
        for future in as_completed(future_to_query):
            ...
            try:
                ...
                logger.debug(
                    f"[RESEARCH]   Query done: '{q}' → {len(results)} results "
                    f"| cumulative: {len(raw_results)}"
                )
            except Exception as e:
    ...
    logger.info(
        f"[RESEARCH]   Total raw results fetched: {len(raw_results)} "
        f"({len(queries)} queries in parallel, {search_elapsed:.2f}s)"
    )
...
def worker_node(payload: dict) -> dict:
    ...
    logger.info(
        ...
        f"| LLM time: {worker_elapsed:.2f}s"
    )
...
def generate_and_place_images(state: State) -> dict:
    ...
    # ── Point 3: generate all images concurrently (network I/O-bound → threads) ──
    MAX_IMAGE_RETRIES = 3
    RETRY_BACKOFF_SECONDS = [2, 5, 10]   # wait before attempt 2, 3, 4

    def _generate_one(spec: dict, idx: int) -> dict:
        """Worker fn for ThreadPoolExecutor — retries up to MAX_IMAGE_RETRIES times."""
        import time
        ...
        logger.info(
            f"[IMGPLACE]   Processing image {idx}/{len(image_specs)}: "
            f"placeholder='{placeholder}' | filename='{filename}'"
        )

        if out_path.exists():
            logger.info(
                f"[IMGPLACE]   Image already exists on disk, skipping generation: '{out_path}'"
            )
        ...
        for attempt in range(1, MAX_IMAGE_RETRIES + 1):
            if attempt > 1:
                ...
                logger.warning(
                    f"[IMGPLACE]   Image {idx} retry {attempt}/{MAX_IMAGE_RETRIES} "
                    f"for placeholder='{placeholder}' — waiting {wait}s before retry..."
                )
                time.sleep(wait)

            logger.debug(
                f"[IMGPLACE]   Generating image via Gemini (parallel) — "
                f"attempt {attempt}/{MAX_IMAGE_RETRIES}..."
            )
            img_gen_start = datetime.now()
            try:
                img_bytes = _gemini_generate_image_bytes(spec["prompt"])
                out_path.write_bytes(img_bytes)
                elapsed = (datetime.now() - img_gen_start).total_seconds()
                logger.info(
                    f"[IMGPLACE]   Image {idx} saved: '{out_path}' "
                    f"({len(img_bytes)} bytes, {elapsed:.2f}s, attempt {attempt}/{MAX_IMAGE_RETRIES})"
                )
                return {"spec": spec, "status": "success"}
            except Exception as e:
                last_exc = e
                logger.warning(
                    f"[IMGPLACE]   Image {idx} attempt {attempt}/{MAX_IMAGE_RETRIES} FAILED "
                    f"for placeholder='{placeholder}': {e}"
                )

        # All retries exhausted
        logger.error(
            f"[IMGPLACE]   Image {idx} PERMANENTLY FAILED after {MAX_IMAGE_RETRIES} attempts "
            f"for placeholder='{placeholder}'. Last error: {last_exc}",
            exc_info=True,
        )
        return {"spec": spec, "status": "failed", "error": last_exc}

    img_gen_start_all = datetime.now()
    results_map: dict = {}   # placeholder → result dict
    ...
    img_gen_elapsed = (datetime.now() - img_gen_start_all).total_seconds()
    logger.info(
        f"[IMGPLACE]   All image generation futures completed in {img_gen_elapsed:.2f}s"
    )

    # ── Apply results to markdown in placeholder order (deterministic) ────────
    ...
    for spec in image_specs:
        ...
        if status == "cached":
            ...
            logger.debug(f"[IMGPLACE]   Placed cached image for '{placeholder}'.")
    ...
    logger.info(
        ...
        f"success={success_count} | failed={failure_count} | cached={cached_count}"
    )
...
def run(topic: str):
    ...
    # operator.add accumulates section tuples across subgraph boundaries,
    # causing duplicates. Deduplicate by task ID to get the true section count.
    sections = out.get('sections', [])
    unique_sections = len({task_id for task_id, _ in sections})
    logger.info("=" * 65)
    ...
    logger.info(f"[RUN] Sections written: {unique_sections}")
    logger.info(f"[RUN] Parallelism     : Tavily searches + image generation run concurrently via ThreadPoolExecutor")
    logger.info("=" * 65)

    return out


query = "Write a blog on Claude AI."
response = run(query)
