...
decision_llm = ChatOllama(model="qwen3:0.6b", temperature=0.7)
...
- If a published date is explicitily present in the result payload, keep it as YYYY-MM-DD.
...
- Mode open_book:
  ...
  - If evidence is empty or insufficient, create a plan that transparently says "insufficient sources" and includes only what can be supported
...
def worker_node(payload: dict) -> dict:
    '''Create worker nodes and work parallely'''
    # payload contains waht we sent
...
def reducer_node(state: State) -> dict:
    '''Combine/stiches all result of different workers'''
    plan = state["plan"]
...
query = "Write a blog on Self Attention"
run(query)
