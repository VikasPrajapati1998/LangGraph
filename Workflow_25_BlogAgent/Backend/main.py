...
# ──────────────────────────────────────────────────────────────
# STATIC FILE SERVING — Blog Images
# ──────────────────────────────────────────────────────────────
from fastapi.staticfiles import StaticFiles

# Serve images from Backend/output/images/ at URL /images/
images_dir = os.path.join(os.path.dirname(__file__), "output", "images")

if os.path.exists(images_dir):
    app.mount("/images", StaticFiles(directory=images_dir), name="images")
    print(f"✅ Images served from: {images_dir} → /images/")
else:
    print("⚠️  output/images folder not found - images will not load")
